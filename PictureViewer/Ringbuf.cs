...
namespace PictureViewer
{
  ...
  /// <typeparam name="T">要素の型</typeparam>
    ...
    public CircularBuffer(int capacity, int width, int height)
    {
      ...
      this.width = width;
      this.height = height;
      this.savedir = @"E:\piccolo\"; //@"C:\piccolo\";
    ...
    public void VideoWriterFrame2()
    {
        // 文字入れ
        // 画面表示
        double gx = this.data[this.bottom].gx;
        double gy = this.data[this.bottom].gy;
        double kgx = this.data[this.bottom].kgx;
        double kgy = this.data[this.bottom].kgy;
        double kvx = this.data[this.bottom].kvx;
        double kvy = this.data[this.bottom].kvy;
        double vmax = this.data[this.bottom].vmax;
        int cr = 5 + (int)(Math.Sqrt(vmax));
        String str = String.Format("ID:{0,6:D1} ", this.data[this.bottom].id) + this.data[this.bottom].t.ToString("yyyyMMdd_HHmmss_fff") + String.Format(" ({0,6:F1},{1,6:F1})({2,6:F1}) K({3,6:F1},{4,6:F1})({5,6:F1},{6,6:F1})", gx, gy, vmax, kgx, kgy,kvx,kvy);

        //Cv.CvtColor(this.img[this.bottom], imgBGR, ColorConversion.GrayToBgr);
        Cv.Copy(this.img[this.bottom], imgR);
        //foreach (CvBlob item in this.data[this.bottom].blobs.Values)
        //{
        //    imgR.Rectangle(item.Rect, new CvColor(0, 100, 200));
        //    imgR.Circle(new CvPoint((int)(gx + 0.5), (int)(gy + 0.5)), cr, new CvColor(0, 100, 255));
        //}

        imgR.Circle(new CvPoint((int)(gx + 0.5), (int)(gy + 0.5)), cr, new CvColor(0, 100, 255));
        imgR.Circle(new CvPoint((int)(kgx + 0.5), (int)(kgy + 0.5)), 30, new CvColor(0, 100, 200));
        imgR.PutText(str, new CvPoint(6, 14), font, new CvColor(0, 100, 120));
    //    Cv.Merge(this.img[this.bottom],this.img[this.bottom],imgR,null,imgBGR);
        //Cv.CvtColor(this.img[this.bottom], imgBGR, ColorConversion.GrayToBgr);

    //    vw.WriteFrame(imgBGR);
    }
